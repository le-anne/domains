...
declare type CustomizationOptionsType = {
    colorPrimary?: string;
...
declare type StyleObject = Partial<CSSStyleDeclaration>;

declare type IFrameCommunicatorProps = {
    link: string;
    iframeId: string;
...
declare class LocalStorage {
    ...
    saveDeviceShare(share: string): Promise<void>;
    getDeviceShare(): Promise<string | null>;
    removeDeviceShare(): Promise<boolean>;
}

declare type ContractCallInputType = {
    contractAddress: string;
    methodInterface: `function ${string}(${string})${string}` | string;
    methodArgs: Array<unknown>;
};
declare type CallContractReturnType = {
    transactionHash: string;
};

declare type GaslessTransactionQuerierTypes = {
    callContract: {
        contractAddress: string;
        method: {
            stub: string;
...
declare type SignerProcedureTypes = {
    getAddress: void;
    signMessage: {
...
declare type WalletManagementTypes = {
    createWallet: void;
    setUpNewDevice: void;
    getUserStatus: void;
};
declare type WalletManagementUiTypes = {
    createWalletUi: void;
...
declare type Chain = "Polygon" | "Mumbai" | "Goerli" | "Ethereum" | "Avalanche";
declare type ClientIdConstructorType = {
    clientId: string;
};
declare type ClientIdWithQuerierType = ClientIdConstructorType & {
    querier: EmbeddedWalletIframeCommunicator<any>;
};
declare type ClientIdWithQuerierAndChainType = ClientIdWithQuerierType & {
    chain: Chain;
};
declare type PaperConstructorType = ClientIdConstructorType & {
    chain: Chain;
...
declare type WalletAddressObjectType = {
    walletAddress: string;
};
declare type SetUpWalletReturnType = WalletAddressObjectType & {
    initialUserStatus: UserWalletStatus;
};
declare type SetUpWalletRpcReturnType = WalletAddressObjectType & {
    deviceShareStored: string;
    isIframeStorageEnabled: boolean;
};
declare type SendEmailOtpReturnType = {
    isNewUser: boolean;
};
declare type LogoutReturnType = {
    success: boolean;
};
declare type GetAuthDetailsReturnType = {
    authDetails?: AuthDetails;
};
declare type GetUserWalletStatusRpcReturnType = {
...
declare type GetUserWalletStatusFnReturnType = {
    status: UserWalletStatus.LOGGED_OUT;
    user: undefined;
} | {
    status: UserWalletStatus.LOGGED_IN_WALLET_UNINITIALIZED;
...
declare const PAPER_APP_URL: string;
declare const EMBEDDED_WALLET_PATH = "/sdk/2022-08-12/embedded-wallet";
declare const EMBEDDED_WALLET_OTP_PATH: string;
declare const EMBEDDED_WALLET_CREATE_WALLET_UI_PATH: string;
declare const EMBEDDED_WALLET_SET_UP_NEW_DEVICE_UI_PATH: string;
declare const AUTH_TOKEN_LOCAL_STORAGE_PREFIX = "walletToken";
declare const AUTH_TOKEN_LOCAL_STORAGE_NAME: (clientId: string) => string;
declare const DEVICE_SHARE_LOCAL_STORAGE_PREFIX = "a";
declare const DEVICE_SHARE_LOCAL_STORAGE_NAME: (clientId: string) => string;
...
declare type GetSocialLoginClientIdReturnType = {
    clientId: string;
};
declare type StoredTokenType = {
    jwtToken: string;
    authProvider: AuthProvider;
    authDetails: AuthDetails;
    developerClientId: string;
};
declare type AuthStoredTokenReturnType = {
    storedToken: StoredTokenType;
};
declare type AuthStoredTokenWithCookieReturnType = {
    storedToken: StoredTokenType & {
        cookieString: string;
...
declare type AuthAndWalletRpcReturnType = AuthStoredTokenWithCookieReturnType & {
    walletDetails: SetUpWalletRpcReturnType;
};
declare type AuthLoginReturnType = {
    user: InitializedUser;
};

declare type GetAddressReturnType = {
    address: string;
};
declare type SignMessageReturnType = {
    signedMessage: string;
};
declare type SignTransactionReturnType = {
    signedTransaction: string;
};

declare type AuthQuerierTypes = {
...
declare class Auth {
     ...
     */
    sendPaperEmailLoginOtp({ email, }: AuthQuerierTypes["sendPaperEmailLoginOtp"]): Promise<{
        isNewUser: boolean;
    }>;
    /**
     *  @description
...
export { AUTH_TOKEN_LOCAL_STORAGE_NAME, AUTH_TOKEN_LOCAL_STORAGE_PREFIX, AuthAndWalletRpcReturnType, AuthDetails, AuthLoginReturnType, AuthProvider, AuthStoredTokenReturnType, AuthStoredTokenWithCookieReturnType, CallContractReturnType, Chain, ChainToPublicRpc, ClientIdConstructorType, ClientIdWithQuerierAndChainType, ClientIdWithQuerierType, ContractCallInputType, DEVICE_SHARE_LOCAL_STORAGE_NAME, DEVICE_SHARE_LOCAL_STORAGE_PREFIX, EMBEDDED_WALLET_CREATE_WALLET_UI_PATH, EMBEDDED_WALLET_OTP_PATH, EMBEDDED_WALLET_PATH, EMBEDDED_WALLET_SET_UP_NEW_DEVICE_UI_PATH, GetAddressReturnType, GetAuthDetailsReturnType, GetSocialLoginClientIdReturnType, GetUser, GetUserWalletStatusFnReturnType, GetUserWalletStatusRpcReturnType, InitializedUser, LogoutReturnType, PAPER_APP_URL, PaperConstructorType, PaperEmbeddedWalletSdk, SendEmailOtpReturnType, SetUpWalletReturnType, SetUpWalletRpcReturnType, SignMessageReturnType, SignTransactionReturnType, StoredTokenType, UserStatus, UserWalletStatus, WalletAddressObjectType, getPaperOriginUrl };
